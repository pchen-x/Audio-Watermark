...
# Routine to extract name of the file from its path without its extension4
# 返回路径下文件名的不带扩展名部分，并加上后缀
def withoutExtensionFile(pathname, sufix=""):
    pathname = os.path.basename(pathname).split(".")
    # print(pathname)
    return pathname[0] + sufix # + '.' + pathname[1]
...
def subVectors(coeff):
    # print(coeff)
    coeffsLen = len(coeff) // 2  # “//”，向下取整，3//2=1
