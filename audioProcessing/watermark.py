...
from .audioTransform import frameToAudio, audioToFrame
import sys
sys.path.append("..")
from universalTools.utils import *
from imageProcessing.imageTransform import binarization, grayscale, imgSize
...
def isImgBinary(image):
    if image.mode != "1":  # 图像不是二进制
        image = binarization(image)
    return image


# Embedding of width and heigth. Audio must be linear and not frames音频文件必须是线性而不是帧
    ...
    for i in range(width):
        for j in range(heigth):
            ...
            for b in range(0, 16, 2):
                joinCoeffs[x][b] = setGray(joinCoeffs[x][b], value)
...
# if __name__ == "__main__":
    # audio = [1, 5, 6, 7, 8, 9, 4, 5, 6, 1, 3, 5, 4, 7, 1, 5, 6, 7, 8, 9, 4, 5, 6, 1, 3, 5, 4, 7, 1, 5, 6, 7, 8, 9, 4, 5,
    #          6, 1, 3, 5, 4, 7, 5, 6, 7]
    ...
    # # print(np.asarray(ideltaDCT(audio)))
    #
